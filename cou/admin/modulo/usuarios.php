...
class Modulo_Usuarios extends Sistema_Modulo{
	...
	public function acaoListarUsuarios(){
		...
		$sql = "SELECT
						...
						grupo.grp_nome,
						status.stt_nome,
						st2.stt_nome as stnome2
				FROM usuario
				LEFT JOIN comprovante ON
					comprovante.usr_cod = usuario.usr_cod
				LEFT JOIN status st2 ON  
					st2.stt_cod = comprovante.stt_cod
				LEFT JOIN participante ON participante.usr_cod = usuario.usr_cod
				LEFT JOIN status ON  status.stt_cod = usuario.stt_cod
		...
		$lista->setColuna("stt_nome","Status Usuario");
		$lista->setColuna("stnome2","Status Comrpovante");
		...
		$lista->setWhere(" usuario.grp_cod != ".DESENVOLVEDOR);
