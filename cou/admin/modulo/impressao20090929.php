	...
	public function acaoImprimircrachas()
	{
		...
		*/
		...
		foreach($participantes as $k=>$participante)
		{
			...
			$cracha .= "<b class='nomeCompleto'>".ucwords(strtolower($participante['usr_nome']))."</b><br/>\n"; 
			$cracha .= "<b class='numero'>".ucwords($participante['usr_cod'])."</b>\n"; 
			$cracha .= "</div>\n";
	...
	public function acaoImprimircertificadoapresentacao()
	{
		...
		if(!$imprimir){
			...
			SELECT
				...
				UCASE(trb_orientador) as trb_orientador,
				trb_coautor
			FROM trabalho t 
			WHERE
				(
			...
			if(is_array($trabalhos)){				
				foreach($trabalhos as $trabalho){
					...
					$autor = $trabalho['trb_apresentador'];
					
					if($trabalho['trb_coautor']){
						$autor .= "; ".$trabalho['trb_coautor'];
					}
					
					if($trabalho['trb_orientador']){
						$autor .= "; ".$trabalho['trb_orientador'];
					}
