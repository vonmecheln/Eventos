...
class Sistema_Layout extends Smarty{
	...
	public function setBotoes($nome,$link,$img,$url=true){
		if($url){
			$botao = sprintf("<a href='%s' title='%s'><img src='%s' border=0 align='absmiddle' >&nbsp;%s</a>",$link,$nome,SISTEMA_URL.$img,$nome);
		}else{
			$botao = sprintf("<a href='javascript:%s' title='%s'><img src='%s' border=0 align='absmiddle' >&nbsp;%s</a>",$link,$nome,SISTEMA_URL.$img,$nome);
		}
		$this->_botoes[] = $botao;	
	}
